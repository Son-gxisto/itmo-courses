...
public class FrontServlet extends HttpServlet {
    ...
    private HashMap<Class, Object> mapInit() {
        HashMap<Class, Object>  res = new HashMap<>();
        res.put(Map.class, null);
        res.put(HttpServletRequest.class, null);
        return res;
    }
    ...
    private void process(Route route, HttpServletRequest request, HttpServletResponse response) throws NotFoundException, ServletException, IOException {
        ...
        for (Class<?> clazz = pageClass; method == null && clazz != null; clazz = clazz.getSuperclass()) {
            for (Method met : clazz.getDeclaredMethods()) {
                ...
                if (met.getName().equals(route.getAction())) {
                    for (Class<?> parClass : met.getParameterTypes()) {
                        if (!allowedPars.containsKey(parClass)) {
                            flag = true;
                            break;
                        }
                    }
        ...
        allowedPars.put(Map.class, view);
        allowedPars.put(HttpServletRequest.class, request);
        List<Object> args = new ArrayList<>();
        method.setAccessible(true);
        try {
            for (Class<?> parClass : method.getParameterTypes()) {
                args.add(allowedPars.get(parClass));
            }
            method.invoke(page, args.toArray());
            if (request.getRequestURI().equals("/ticTacToe") &&
                    (request.getParameterMap().size() > 1 ||
                            request.getParameterMap().size() == 1 && request.getParameter("action") == null)) {
        ...
        if (lang != null && lang.length() == 2 && isLowerLetter(lang.charAt(0)) && isLowerLetter(lang.charAt(1))) {
            request.getSession().setAttribute("lang", lang);
        }
        lang = (String) request.getSession().getAttribute("lang");
        if (lang != null && !"en".equals(lang)) {
            templateName += "_" + request.getSession().getAttribute("lang");
        }
        Template template;
        try {
            template = newTemplate(templateName + ".ftlh");
        } catch (ServletException e) {
            template = newTemplate(templateName.substring(0, templateName.length() - 3) + ".ftlh");
        }
