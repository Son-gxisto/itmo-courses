...
public class UserRepositoryImpl implements UserRepository {
    ...
    public void update(User user) {
        try (Connection connection = DATA_SOURCE.getConnection()) {
            try (PreparedStatement statement = connection.prepareStatement("UPDATE `User` SET `admin` = '" + (user.getAdmin() ? 1 : 0) + "' WHERE `User`.`id` = " + user.getId(), Statement.RETURN_GENERATED_KEYS)) {
