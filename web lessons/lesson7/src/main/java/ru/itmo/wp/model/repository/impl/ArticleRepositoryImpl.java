...
public class ArticleRepositoryImpl implements ArticleRepository {
    ...
    public void save(Article article) {
        try (Connection connection = DATA_SOURCE.getConnection()) {
            try (PreparedStatement statement = connection.prepareStatement("INSERT INTO `Article` (`title`, `text`, `userId`, `creationTime`) VALUES (?, ?, ?, NOW())", Statement.RETURN_GENERATED_KEYS)) {
                ...
                statement.setString(3, String.valueOf(article.getUserId()));
    ...
    public void update(Article article) {
        try (Connection connection = DATA_SOURCE.getConnection()) {
            try (PreparedStatement statement = connection.prepareStatement("UPDATE `Article` SET `hidden` = \'" + (article.getHidden() ? 1 : 0) + "' WHERE `Article`.`id` = " + article.getId(), Statement.RETURN_GENERATED_KEYS)) {
                if (statement.executeUpdate() != 1) {
    ...
    public List<Article> findAllByUser(long userId){
        List<Article> articles = new ArrayList<>();
        try (Connection connection = DATA_SOURCE.getConnection()) {
            try (PreparedStatement statement = connection.prepareStatement("SELECT * FROM `Article` WHERE `userId`=" + userId)) {
                try (ResultSet resultSet = statement.executeQuery()) {
                    Article article;
                    while ((article = toArticle(statement.getMetaData(), resultSet)) != null) {
                        articles.add(article);
